...
namespace SkillPedia\LaravelH5p\Repositories;

use Carbon\Carbon;
use DB;
use SkillPedia\LaravelH5p\Eloquents\H5pContent;
use SkillPedia\LaravelH5p\Eloquents\H5pContentsLibrary;
use SkillPedia\LaravelH5p\Eloquents\H5pContentsUserData;
use SkillPedia\LaravelH5p\Eloquents\H5pLibrariesLibrary;
use SkillPedia\LaravelH5p\Eloquents\H5pLibrary;
use SkillPedia\LaravelH5p\Eloquents\H5pResult;
...
//                    DB::select("SELECT semantics FROM h5p_libraries WHERE name = ? AND major_version = ? AND minor_version = ?", [$name, $majorVersion, $minorVersion]);
        ...
        return ($semantics === false ? null : (isset($semantics->semantics) ? $semantics->semantics : null));
    }
