...
use SkillPedia\LaravelH5p\Exceptions\H5PException;
use Response;
use SkillPedia\LaravelH5p\Eloquents\H5pTmpfile;
...
class H5pController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('h5p.content.index', compact('settings', 'entrys', 'request', 'search_fields'));
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        return view('h5p.content.edit', compact('settings', 'user', 'id', 'content', 'library', 'parameters', 'display_options','nonce'));
    }
    ...
    public function show(Request $request, $id)
    {
        ...
        return response()
            ->view('h5p.content.show', compact('settings', 'user', 'embed_code', 'id'))
    ...
    private function handle_upload_nonce($nonce, $contentId)
    {
        ...
        foreach ($files as $file) {
            if (strpos($file->path, 'editor') !== false) {
                ...
                $destPath = storage_path('h5p' . $destFile);
                $destDir = dirname($destPath);
                if (!is_dir($destDir)) {
                    mkdir($destDir, 0777, true);
                }
                rename(storage_path('h5p' . $file->path), $destPath);
